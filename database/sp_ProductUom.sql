...
BEGIN
	...
	SET @s =  (concat ('select product_id,product_code,product_name,item_no,uom_id,uom_code,ratio,next_uom_id,next_uom_code,is_default,price from (
							select product_id,product_code,product_name,item_no,uom_id,uom_code,ratio,lag(uom_id) over (partition by product_id order by item_no) next_uom_id,is_default,price
							from product_uom t0
