...
DELIMITER $$
CREATE PROCEDURE sp_Company(updatedAt date,uId int,companyTypeId int,statusIds varchar(5),userId int, keyword varchar(255), in TheField varchar(8000),in MatchMode varchar(8000),in ValueName varchar(8000),in limitVal int, in offsetVal int )
BEGIN
	...
	SET @s =  (concat ("select * from (
						...
						where deleted_at is null ",uIdSet,updatedAtSet,keywordSet,companyTypeIdSet,statusIdSet,") x where id != 0 ",ColumnSet,limitSet,offsetSet," ;"));
