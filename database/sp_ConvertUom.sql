...
BEGIN
    ...
    select product_id,product_code,product_name,price,qty,uom_id,uom_code,ratio,packaging_id,packaging_code,final_qty,final_uom_id,final_uom_code,conversion_qty,conversion_uom_id,conversion_uom_code from (
    select uId product_id,product_code,product_name,price,qtyId qty,t0.uom_id,case when uomId = final_uom_id then t3.is_default_ratio else ratio end ratio,case when uomId = final_uom_id then uomId else lag(t0.uom_id) over (partition by product_id order by item_no) end packaging_id,qtyId*t0.is_default_ratio final_qty,final_uom_id,final_uom_code, qtyId*final_ratio conversion_qty,t2.conversion_uom_id,t2.conversion_uom_code
    from product_uom t0 
