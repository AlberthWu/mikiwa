...
BEGIN
    ...
    if reportTypeId = 1 then
		...
		SET @s =  (concat ("select count(id) data0,sum(open) `open`, sum(unposted) unposted, sum(posted) posted, sum(`open`+unposted+posted) total from (select id,code_coa,name_coa,company_id,company_code,opening,debet,credit,balance,`open`,unposted,posted,
							...
							from 
								(select id,code_coa,name_coa,company_id,company_code,t1.opening,t1.debet,t1.credit,t1.balance,t1.`open`,t1.unposted,t1.posted
									...
									where deleted_at is null and status_id = 1 and id not in (select parent_id from chart_of_accounts)  and is_header = 1 ",accountIdSet,userIdSet,transactionOnlySet,companyIdSet,") x where id != 0 ",ColumnSet,") y ;"));
	elseif reportTypeId = 2 then
        ...
        if ifnull(updatedAtSet,'') = '' then
			SET @s =  (concat ("select count(id) data0 from (select urut,id,issue_date,company_id,company_code,voucher_seq_no,voucher_code,voucher_no,account_id_header,account_code_header,account_name_header,debet,credit,balance,transaction_type,status_id,status_desc,status_gl_id,status_gl_desc,updated_by,status_data ,
								...
								from (
									...
									from 
										...
										order by account_id_header,urut,period,voucher_seq_no,id) y where account_id_header != 0  ",ColumnSet,") z;"));
		else
			SET @s =  (concat ("select count(id) data0 from (select urut,id,issue_date,company_id,company_code,voucher_seq_no,voucher_code,voucher_no,account_id_header,account_code_header,account_name_header,debet,credit,balance,transaction_type,status_id,status_desc,status_gl_id,status_gl_desc,updated_by,status_data ,
									'issue_date,company_code,voucher_code,voucher_no,account_code_header,account_name_header,debet,credit,balance,transaction_type,status_id,status_gl_id,updated_by,status_data' field_key,
									'Tgl,Grp PT,Kode voucher,No voucher,Kode, Nama,Debet,Kredit,Akhir,Transaksi,Status, Status gl,User,Status data' field_label,
