...
BEGIN
    ...
    if reportTypeId = 1 then
		...
		SET @s =  (concat ("select count(id) rn from (select id,code_coa,name_coa,opening,debet,credit,balance,
							...
							from 
								(select id,code_coa,name_coa,t1.opening,t1.debet,t1.credit,t1.balance
									...
									where deleted_at is null and status_id = 1 and id not in (select parent_id from chart_of_accounts)  and is_header = 1 ",accountIdSet,userIdSet,transactionOnlySet,") x where id != 0 ",ColumnSet,") y ;"));
	elseif reportTypeId = 2 then
		...
		SET @s =  (concat ("select count(id) rn from (select urut,id,issue_date,voucher_seq_no,voucher_code,voucher_no,account_id_header,account_code_header,account_name_header,debet,credit,balance,transaction_type,status_id,status_desc,status_gl_id,status_gl_desc,updated_by,status_data ,
