...
BEGIN
    ...
    SET keywordSet  = case when (keyword is null or keyword = '') then '' else concat(" and (t0.product_code like '%",keyword,"%'  or t0.product_name like '%",keyword,"%' or t0.product_division_code like '%",keyword,"%' or t0.product_division_name like '%",keyword,"%' 
		 or t0.product_type_name like '%",keyword,"%'   or t0.uom_code like '%",keyword,"%' or t0.art_no like '%",keyword,"%'  or t0.barcode like '%",keyword,"%')")   end;
    ...
    if reportType = 0 then
		...
		SET @s =  (concat ("select count(id) rn from (select id,product_code,product_name,serial_number,art_no,barcode,product_type_id,product_type_name,product_division_id,product_division_code,product_division_name,uom_id,uom_code,lead_time,status_id,status_data,is_purchase,is_sales,is_production
							...
							from
								...
								left join
									(select id,is_purchase,is_sales,is_production from product_types) t1 on t1.id = t0.product_type_id
								where deleted_at is null ",uIdSet,updatedAtSet,keywordSet,divisionIdSet,typeIdSet,statusIdSet,purchaseIdSet,salesIdSet,productionIdSet,") x where id != 0 ",ColumnSet,") y ;"));
	end  if;
